...
- `?key=vulcx_your_key_here` query parameter — **required** (not optional) for the WebSocket
  stream (`GET /api/v1/stream`), since a browser WS handshake can't carry custom headers.
...
An invalid key returns `403 Forbidden` (invalid/disabled/revoked). `401 Unauthorized` only occurs
on the WebSocket stream with no key. Keys are free during beta — see [Authentication](https://docs.vulcx.xyz/get-started/authentication) for how to get one.
...
|----------|--------|:---:|---------|
| `/api/v1/quote` | GET | optional | Get best route and estimated output |
| `/api/v1/swap` | POST | optional | Build unsigned swap transaction |
| `/api/v1/instructions` | POST | optional | Get raw instructions for custom tx |
| `/api/v1/cpi/route-accounts` | POST | optional | Build the `route` instruction for on-chain CPI (PDA-signed) — Beta |
...
| sessionAccount | string | no | -- | Fogo session account: returns ONE session-shaped route ix, no ATA-create/wrap ixs; response gains requiredTokenAccounts; Valiant-V1-only |
...
| Error | HTTP | Recovery |
|-------|------|----------|
| `missing api key` | 401 | Only occurs on the WS stream — add `?key=<key>` |
