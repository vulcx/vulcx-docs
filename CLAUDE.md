...
Mintlify documentation site for **Vulcx** — a swap aggregator / routing engine on **Fogo Chain**. This is the public developer docs surface; it documents sibling projects that live elsewhere in the `mono_vulcx` umbrella:
...
The `widget` (`@vulcx/widget`) tab and its pages were removed from this site (not enough
bandwidth to keep it maintained) — don't re-add a Widget tab or `widget/*` pages without
being asked; the SDK is the only documented client library now.
...
**`docs.json` is the control plane.** It defines theme, colors, fonts, SEO/OG metadata, the OpenAPI source, and — most importantly — `navigation.tabs`. The site has top-level tabs (Get Started, Docs, SDK, API Reference, Guides, AI, Updates), each containing groups that list pages by path (no extension).
...
- Pages are **MDX** under most tabs; the `sdk/` pages are plain **`.md`**. Both use YAML frontmatter.
...
- **Venue naming: the brand is Valiant; `Vortex` is only the wire value.** The API reports
  ...
  Everywhere a human is being told which venues Vulcx routes across, write **Valiant**.
  The CPI guide keeps `Vortex` throughout — `VortexHop`, `Hop::Vortex`, `vortex_program` and the
  "Vortex (7)" / "Vortex V2 (10)" account-group tabs are Rust identifiers and account layouts, and
  the prose around that code block matches them on purpose. If you see "Vortex" naming the venue in
  ordinary copy, it is a regression.
- `llms.txt` is **generated** — run `python3 scripts/gen-llms-txt.py` after editing page frontmatter
  rather than hand-editing it. The header blurb lives in the script itself. `llms-full.txt` and
  `skill.md` are hand-maintained and must be updated separately.
