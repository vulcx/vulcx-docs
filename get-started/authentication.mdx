...
<Steps>
  <Step title="Request a key">
    ...
    <Card title="Get an API key on Discord" icon="discord" href="https://discord.gg/z98Zmk6chP" horizontal>
      Join the Vulcx Discord and request a key.
    </Card>
  </Step>
  <Step title="Store it as an environment variable">
    ...
    ```bash
    export VULCX_KEY="argy_your_key_here"
    ```
  </Step>
</Steps>
...
```html Widget
<vulcx-swap api-key="argy_your_key_here" chain="fogo" theme="dark"></vulcx-swap>
```
</CodeGroup>
...
```text
wss://api.vulcx.xyz/api/v1/stream?key=argy_your_key_here
```
