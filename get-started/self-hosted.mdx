...
when it opens, and use the hosted API at `api.vulcx.xyz` until then — the Free plan
costs nothing and keyless requests work for trying it.
</Warning>
...
| Rate limits | Per-account budget set by your plan | Built into the binary, per license |
| Infrastructure | Managed by Vulcx | Your server |
| RPC provider | Managed by Vulcx | FluxRPC (your key) |
| Cost | Free plan, or a paid plan from $19/month for a larger budget and the stream | Your own RPC and server. The community licence is free during beta |
