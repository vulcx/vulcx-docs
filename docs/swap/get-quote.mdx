...
<ResponseField name="amountOut" type="string">Output amount in smallest units — what the route actually delivers. For `ExactIn` this is the amount the route produces. For `ExactOut` it is the amount you asked for plus Vulcx's fee, which comes out of the output — so what you receive is exactly your `amount` (a 10000 request at 15 bps shows `amountOut` 10015). See [Partial fills](#partial-fills): it can come back lower.</ResponseField>
...
<ResponseField name="routes" type="RouteInfo[]">
  ...
  <Expandable title="RouteInfo fields">
    ...
    <ResponseField name="poolType" type="string">Pool type. On Fogo the wire values are `Vortex` (the venue branded Valiant) and `Flux` (Fluxbeam). `Cyclone` is reserved and not yet routable.</ResponseField>
