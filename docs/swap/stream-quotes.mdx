...
<CodeGroup>
```javascript Browser
const ws = new WebSocket(`wss://api.argyros.xyz/api/v1/stream?key=${ARGYROS_KEY}`);
...
const ws = new WebSocket(`wss://api.argyros.xyz/api/v1/stream?key=${process.env.ARGYROS_KEY}`);
