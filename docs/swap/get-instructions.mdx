...
description: "Get raw swap instructions from POST /api/v1/instructions to compose Vulcx swaps with your own logic — memos, account creation, batching — into one transaction."
llmDescription: "Detailed guide for POST /api/v1/instructions on SVM chains (live on Fogo). Explains when to use instructions instead of swap (custom transaction composition, adding memos, creating accounts). Covers request body (including optional quoteId and firm for firm-quote redemption, referrer as the wallet the caller's own integratorFeeBps is paid to (kept in full, not a share of Vulcx's rate), and sessionAccount for Fogo Sessions — session-shaped single route instruction, no ATA-create/wrap instructions, requiredTokenAccounts in the response, Valiant-V1-only), response schema (InstructionsResponse with RawInstruction array, addressLookupTableAddresses, requiredTokenAccounts), and complete TypeScript example for building a v0 transaction from raw instructions with Address Lookup Tables."
---
...
<ParamField body="referrer" type="string">
  Optional referrer wallet (base58). Receives `integratorFeeBps` **in full**, paid on-chain in the
  output token in the same transaction as the swap. Required whenever `integratorFeeBps` is set.
</ParamField>

<ParamField body="integratorFeeBps" type="integer" default="0">
  Your own fee rate, in basis points of the output amount — **you keep 100% of it**. It is added to
  Vulcx's `platformFeeBps`, not carved out of it, and the sum may not exceed **100 bps** (`400`
  otherwise). Requires `referrer`. See [Fees](/concepts/fees).
</ParamField>

<ParamField body="sessionAccount" type="string">
  Optional Fogo session account (base58). Builds a **session-shaped** route: the session account is
  the signing authority (`userWallet`'s ATAs still hold the funds), and no ATA-create or SOL-wrap
  instructions are emitted — the response's `requiredTokenAccounts` lists what must already exist.
  Send via the Fogo Sessions SDK (session key signs, paymaster pays). Session routes are currently
  Valiant-V1-only.
</ParamField>
...
<ResponseField name="amountOut" type="string">Output amount in smallest units, before fees. You receive `amountOut - platformFeeAmount - integratorFeeAmount`. For `ExactOut` the route is sized so that difference is exactly the `amount` you asked for.</ResponseField>
...
<ResponseField name="feeAmount" type="string">DEX pool trading fee across every hop — the LP fee the pools take, not Vulcx's.</ResponseField>
<ResponseField name="platformFeeBps" type="integer">Vulcx's fee rate, in bps of the output. `0` when the protocol fee is disabled on-chain or waived for you.</ResponseField>
<ResponseField name="platformFeeAmount" type="string">`platformFeeBps` applied to `amountOut`, in output token units.</ResponseField>
<ResponseField name="integratorFeeBps" type="integer">Your own fee rate, echoed back. Independent of `platformFeeBps` — the two add. See [Fees](/concepts/fees).</ResponseField>
<ResponseField name="integratorFeeAmount" type="string">`integratorFeeBps` applied to `amountOut`, paid on-chain to your `referrer` wallet.</ResponseField>
...
<ResponseField name="requiredTokenAccounts" type="string[]">
  Session mode only (`sessionAccount` set): the user's ATAs the route touches, ordered
  `[inputMint, …intermediates, outputMint]`. They must exist — and the input ATA hold `amountIn` —
  before the session transaction is sent, since session instructions include no ATA-create or
  SOL-wrap steps.
</ResponseField>
