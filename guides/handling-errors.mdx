...
| `400` | `ExactOut is only supported on Vortex pools; ...` | Route crosses Fluxbeam or Moonit, which have no exact-out CPI | Re-quote as ExactIn |
...
if (!result.success) {
  ...
  if (err === "missing api key" || err === "invalid api key") {
    ...
    console.log("Re-quote as ExactIn.");
  } else {
