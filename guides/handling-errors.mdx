...
| `400` | `ExactOut is only supported on Valiant pools; ...` | Route crosses Fluxbeam, which has no exact-out CPI | Re-quote as ExactIn |
| `422` | `the best route needs more accounts than fit ...` | Route found but too large for one transaction | Split the trade into smaller amounts |
...
if (!result.success) {
  // Branch on `code`, never on `error`. The code is the contract; the message
  // is prose and gets reworded. Codes you have not seen before should fall
  // through to the default rather than be guessed at from the text.
  switch (result.code) {
    ...
    case "NO_ROUTE":
      ...
      break;
    case "EXACT_OUT_MULTIHOP":
      console.log("ExactOut is single-hop only — use ExactIn with a buffer.");
      break;
    case "EXACT_OUT_DEX":
      console.log("This route has no exact-out CPI. Re-quote as ExactIn.");
      break;
    case "QUOTE_EXPIRED":
    case "QUOTE_STALE":
      console.log("The quote aged out. Re-quote and retry.");
      break;
    case "SIM_SLIPPAGE":
      console.log("Price moved past your slippage. Re-quote, or widen it.");
      break;
    default:
      console.error(`API error ${result.code ?? response.status}: ${result.error}`);
  }
}
```
