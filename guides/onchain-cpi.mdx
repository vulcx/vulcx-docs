...
| 1 | `authority` (`user_transfer_authority`) | **signer**, readonly |
...
{ "op": "subscribe_route", "routes": [{
    ...
    "outputMint": "uSd2czE61Evaf76RNbq4KPpXnkiL3irdzgLFUMe3NoG",
    "amount": "1000000000",
    "allowedIntermediateMints": [], "maxHops": 1
}]}
```
...
- **The authority is never writable.** It used to have to be, for Moonit hops. Moonit is gone and no
  route can contain one, so `RouteAccounts` will not ask for a writable authority. Marking your PDA
  `mut` is harmless but no longer needed.
