...
description: "End-to-end Vulcx swap on any supported SVM chain: get a quote, build the transaction, then sign and submit with @solana/web3.js — complete runnable cURL, TypeScript, and Python code."
llmDescription: "Step-by-step guide to executing a token swap on Vulcx on SVM chains (live on Fogo). Covers the full flow: GET /api/v1/quote (with cURL + TypeScript + Python), POST /api/v1/swap to build the transaction, then sign with @solana/web3.js VersionedTransaction, sendRawTransaction, and confirmTransaction. Includes complete runnable code examples."
---
