...
description: "How Vulcx routing works on Fogo: direct, multi-hop (up to 5 hops), and split routes across Vortex, Fluxbeam, and Moonit, with slippage protection."
llmDescription: "How Vulcx routing works at a high level on Fogo. Covers direct swaps, multi-hop routing through intermediate tokens (up to 5 hops), supported liquidity sources (Vortex, Fluxbeam, Moonit bonding curves), price impact severity levels, and slippage protection mechanics."
---
...
<Frame caption="A quote request goes into the routing engine, which compares direct, multi-hop, and split routes across every pool and returns the best-price quote.">
  <img src="/images/routing.png" alt="Vulcx routing: a quote request (tokenIn, tokenOut, amount, mode) enters the Vulcx routing engine, which searches every pool on Vortex, Fluxbeam, and Moonit for the best price. It picks the best of a direct swap, a multi-hop route (up to 5 hops), or a split route across several pools, then returns a quote response (amountOut, price impact, route) that POST /swap turns into an unsigned transaction." />
</Frame>
